...
export default function Index(data: {tr: transaction[], ba: user, user: any}) {
  return (
    ...
    <Head><title>{data.user.username||data.ba.key} — 🎈</title></Head>

    <Bx>
    <Heading as="h1">{data.user.username||data.ba.key}</Heading>
    {data.ba.badges.includes(5) ? '' : <Clink href={'https://forum.gethopscotch.com/u/'+data.user.username||data.ba.key} target="_blank" rel="noreferrer">See {data.user.username||data.ba.key} on the Hopscotch Forum ↗</Clink>}
